...
def buildWorld(modelParameters, mapParameters, agentParameters, networkParameters):
    if mapParameters.filePath is None:
...
# [locationLikelihood, genderLikelihood, ageLikelihood, survivalRate, fertilityRate, ageDiscountRateFactor, agePointsPopulation, agePointsSurvival, agePointsFertility, agePointsPref] = buildDemography(modelParameters, locations);
    ...
    survivalRate = survivalRate**(1/modelParameters.cycleLength) # TODO: make sure survival rate is a single value and not an array
    fertilityRate = 1 - ((1-fertilityRate)**(1/modelParameters.cycleLength)) # TODO: make sure fertility rate is a single value and not an array
...
# fertilityRate = 1 - ((1-fertilityRate).^(1/modelParameters.cycleLength));
    ...
    demographicVariables = {
        'locationLikelihood': locationLikelihood,
        'genderLikelihood': genderLikelihood,
        'ageLikelihood': ageLikelihood,
        'survivalRate': survivalRate,
        'fertilityRate': fertilityRate,
        'agePointsPopulation': agePointsPopulation,
        'agePointsSurvival': agePointsSurvival,
        'agePointsFertility': agePointsFertility,
        'agePointsPref': agePointsPref,
        'ageDiscountRateFactor': ageDiscountRateFactor
    }
...
# demographicVariables.ageDiscountRateFactor = ageDiscountRateFactor;
    ...
    utilityVariables = {
        'numForms': max(utilityForms),
        'utilityLayerFunctions': utilityLayerFunctions,
        'utilityHistory': utilityHistory,
        'utilityAccessCosts': utilityAccessCosts,
        'utilityTimeConstraints': utilityTimeConstraints,
        'utilityAccessCodesMat': utilityAccessCodesMat,
        'utilityBaseLayers': utilityBaseLayers,
        'utilityForms': utilityForms,
        'incomeForms': incomeForms,
        'utilityPrereqs': utilityPrereqs,
        'nExpected': nExpected,
        'hardSlotCountYN': hardSlotCountYN,
        'hasOpenSlots': np.zeros(np.shape(hardSlotCountYN)), # TODO: Check to make sure this is as intended
    }
